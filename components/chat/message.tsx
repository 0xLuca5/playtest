...
import { TestingPreview } from './testing-preview';
      ...
      >
        ...
        >
          {message.role === 'assistant' && (
            <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center mt-1 shrink-0">
          ...
          >
            ...
            {message.parts?.map((part, index) => {
                      ...
                      {state === 'output-available' && (
                        <ToolOutput
                          output={
                            (typeof part.output === 'object' && part.output !== null && 'error' in part.output) ? (
                              <div className="p-2 text-red-500 rounded border">
                                Error creating test case: {String(part.output.error)}
                              </div>
                            ) : typeof part.output === 'string' && part.output.includes('❌') ? (
                              <div className="p-2 text-red-500 rounded border">
                                {part.output}
                              </div>
                            ) : (
                      ...
                      {state === 'output-available' && (
                        <ToolOutput
                          output={
                            (typeof part.output === 'object' && part.output !== null && 'error' in part.output) ? (
                              <div className="p-2 text-red-500 rounded border">
                                Error updating test case: {String(part.output.error)}
                              </div>
                            ) : (
                              typeof part.output === 'object' &&
                              part.output !== null &&
                              'success' in part.output &&
                              (part.output as any).success === true &&
                              'message' in part.output
                            ) ? (
                              <div className="p-2 text-green-600 rounded border border-green-200 bg-green-50 dark:bg-green-950/30 dark:border-green-900">
                                {String((part.output as any).message)}
                              </div>
                            ) : typeof part.output === 'string' && part.output.includes('✅') ? (
                              <div className="p-2 text-green-600 rounded border border-green-200 bg-green-50 dark:bg-green-950/30 dark:border-green-900">
                                {part.output}
              ...
              // Test Case Automation Execution Tool
