...
import { saveChatModelAsCookie } from '@/app/(protected)/chat/actions';
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        return {
          url: `${window.location.origin}${url}`, // 转换为完整URL
          name: pathname,
...
function PureModelSelectorCompact({
  ...
  const { chatModels: configuredChatModels, loading, error, getDefaultChatModel } = useChatModels();

  // 使用配置的chat models，如果没有配置则fallback到默认的chatModels
  const availableModels = configuredChatModels.length > 0 ? configuredChatModels : chatModels;
  ...
  if (loading) {
    return (
      <PromptInputModelSelect disabled>
        <PromptInputModelSelectTrigger
          type="button"
    ...
    <PromptInputModelSelect
      ...
      onValueChange={(modelName) => {
        ...
        if (model) {
          ...
          // 立即设置客户端cookie
          setClientCookie('chat-model', model.id);

          // 立即触发事件
          if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('cookieChange', {
              detail: { name: 'chat-model', value: model.id }
            }));
          }

          // 异步调用服务器端函数（用于服务器端渲染时的一致性）
          startTransition(() => {
            saveChatModelAsCookie(model.id).catch(error => {
              console.error('🔍 saveChatModelAsCookie失败:', error);
            });
          });
        } else {
          console.log('🔍 没有找到匹配的模型:', modelName);
        }
      }}
    >
      <PromptInputModelSelectTrigger
